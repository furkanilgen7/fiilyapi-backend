...
from .accumulate import Resolved, accumulate, resolve_all
...
def compute_daily_report(inp: EngineInput, report_date: date) -> DailyReport:
    ...
    with localcontext(ENGINE_CONTEXT):
        ...
        calendar.position(report_date)  # takvim disi d → ValueError (girislerden ONCE)
        tree = build_tree(inp.nodes)
        return report_from(inp, calendar, tree, None, report_date)
...
def report_from(
    ...
    """`compute_daily_report` govdesi — agac/inisler disaridan verilebilir (panel paylasimi;
    ENGINE_CONTEXT icinde cagrilir). `resolved` None → girisler burada dogrulanir."""
    position = calendar.position(report_date)
    ...
    points = accumulate(tree, calendar, inp.qty_entries, inp.hours_entries, report_date, resolved)
