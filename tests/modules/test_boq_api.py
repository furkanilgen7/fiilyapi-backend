"""BOQ — OKUMA ucu (§5.1) ve T5 YARATMA uçları (grup + kalem).

Görünmeyen şantiye **404** döner, 403 DEĞİL. Yetki kapısı seed değerine BAĞIMLI
değildir: rol izni testin içinde açıkça ayarlanır.

⚠️ Dosya 800 satır tavanını aşınca BÖLÜNDÜ (`_journal.py` emsali): T6 güncelleme
+ T7 denetim `test_boq_update.py`ye, silme uçları `test_boq_delete.py`ye taşındı;
paylaşılan yardımcılar `_boq.py`dedir. Hiçbir testin iddiası değişmedi.
"""
