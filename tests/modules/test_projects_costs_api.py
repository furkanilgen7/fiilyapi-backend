...
async def test_ayni_taseronun_iki_sozlesmesi_tek_satirda_toplanir(
    client, db_session, user_factory, project_factory
):
    """Tablo satırı TAŞERON başınadır (görev emri): iki sözleşme tek satırda toplanır."""
    ...
    birinci = await _contract(
        db_session, project, kurucu, name="Akın İnşaat", item_quantity="1000", item_price="1000"
    )
    await _contract(
        db_session, project, kurucu, name="Akın İnşaat", item_quantity="500", item_price="1000"
    )
    ...
    assert Decimal(row["contract_amount"]) == Decimal("1500000.00")
    assert Decimal(row["paid"]) == Decimal("300000.00")
