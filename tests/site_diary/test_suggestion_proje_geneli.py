...
async def prj_geneli(
    ...
    # --- KARŞI KANIT: BAŞKA PROJENİN günlüğü, bu projenin kalemine köprülü ---
    site_q, _proje_q, pozlar_q = await santiye_fabrikasi("PG-Q", item_specs=_specler("Q1"))
    pozlar_q[0].contract_item_id = szl_ortak.id
    ...
    await _gun(
        ...
        site_q.id,
        date(2026, 7, 10),
        [{"boq_item_id": str(pozlar_q[0].id), "quantity": "50"}],
    )
    ...
    return contract, (site_a, site_b), proje, poz, beklenen
...
async def test_sitesiz_sozlesme_PROJE_TOPLAMI_ve_DISARIDA_KALANLAR(
    ...
    """
    contract, _, _, _, beklenen = prj_geneli
...
async def test_sitesiz_sozlesmede_KOPRUSUZ_pozlar_PROJE_GENELINDE_sayilir(
    ...
    Başka projenin pozu (Q1) ve dönem içi toplamı SIFIR olan poz (A4) SAYILMAZ.
    """
    contract, _, _, _, _ = prj_geneli
